...
static inline std::uniform_int_distribution<int> randChar(0,
                                                          sizeof(alphanum) - 2);

static inline std::random_device rd;
static inline thread_local std::mt19937 gGenerator(rd());

inline void genUID(char* out, size_t size) {
    for (int i = 0; i < size; i++) out[i] = alphanum[randChar(gGenerator)];
}