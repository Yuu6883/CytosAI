...
private:
    ...
    struct QuadNode {
        ...
        QuadNode(QuadTree& tree, IRect rect, QuadNode* root = nullptr):
        ...
        const bool contains(Cell* cell) {
            return rect.toAABB().contains(cell->shared.aabb);
        }
        ...
        void split() {
            ...
            branches = new QuadNode[4] {
                QuadNode(tree, IRect { rect.x - qw, rect.y + qh, qw, qh }, this),
                QuadNode(tree, IRect { rect.x + qw, rect.y + qh, qw, qh }, this),
                QuadNode(tree, IRect { rect.x - qw, rect.y - qh, qw, qh }, this),
                QuadNode(tree, IRect { rect.x + qw, rect.y - qh, qw, qh }, this)
            };
    ...
    QuadNode root;
public:
    QuadTree(IRect rect, uint32_t maxLevel, uint32_t maxItems):
    ...
    inline void query(Cell& cell, bool, const QueryFunc& cb) {
        rts.clear();
        rts.push_back(&root);
...
public:
    ...
    inline void countItems(vector<uint32_t>& out) {
        auto target = std::min(out.size(), size_t(maxLevel));
    ...
    inline void query(TAABB<T>& aabb, const QueryFunc& cb) {
        rts.clear();
        rts.push_back(&root);
