...
TemplateEngine<T>::TemplateEngine(Server* server)
    : Engine(server),
    ...
    this->tree = new Solver(IRect(0, 0, dim, dim), T.QUADTREE_MAX_LEVEL,
                            T.QUADTREE_MAX_ITEMS);
    this->dualEnabled = T.DUAL_ENABLED;
    this->defaultCanSpawn = T.PLAYER_CAN_SPAWN;
    this->desiredBots = T.BOTS;

#ifdef WIN32
    pool = static_cast<Cell*>(_aligned_malloc(poolSize(), sizeof(Cell)));
    boosts = static_cast<Boost*>(_aligned_malloc(boostSize(), sizeof(Boost)));
#else
    pool = static_cast<Cell*>(aligned_alloc(sizeof(Cell), poolSize()));
    boosts = static_cast<Boost*>(aligned_alloc(sizeof(Boost), boostSize()));
#endif

    memset(pool, 0, poolSize());
    memset(boosts, 0, boostSize());
}
...
void Engine::addHandle(Handle* handle, uint16_t cid) {
...
void Engine::removeHandle(Handle* handle) {
    if (!running) return;
    if (!handle->control) return;
...
void TemplateEngine<T>::kill(Control* control, bool replace) {
    constexpr cell_cord_prec R_THRESH = T.PLAYER_INIT_SPAWN_SIZE > 0
                                            ? (100 + T.PLAYER_INIT_SPAWN_SIZE)
                                            : (100 + T.PLAYER_SPAWN_SIZE);
    constexpr float MASS_THRESH = R_THRESH * R_THRESH * 0.01f;
...
void TemplateEngine<T>::spawnViruses() {
    for (uint32_t i = 0; i < T.MAX_VIRUS_PER_TICK; i++) {
        auto idealVirusCount =
            T.VIRUS_COUNT * (1 - (playerMass.load() + botMass.load()) /
                                     (T.MAP_HW * T.MAP_HW * 0.01f));

        if (viruses.size() < idealVirusCount) {
            auto&& point =
                getSafeSpawnPoint(T.VIRUS_SIZE, T.VIRUS_SAFE_SPAWN_RADIUS);
            // if (!success) break;
...
void TemplateEngine<T>::spawnPlayers() {
    ...
    for (auto c : copy) {
        ...
        bool s =
            c->handle->isBot() ? spawnBotControl(c) : spawnPlayerControl(c);
...
void TemplateEngine<T>::spawnEXP() {
    if (exps.size() >= T.MAX_EXP_CELLS) return;
    auto&& point =
        getSafeSpawnPoint(T.EXP_CELL_SIZE, T.PLAYER_SAFE_SPAWN_RADIUS);
...
void TemplateEngine<T>::spawnCYT() {
    if (cyts.size() >= T.MAX_CYT_CELLS) return;
    auto&& point =
        getSafeSpawnPoint(T.CYT_CELL_SIZE, T.PLAYER_SAFE_SPAWN_RADIUS);
    if (!point.safe) return;
...
template <OPT const& T>
void TemplateEngine<T>::spawnWH() {}

template <OPT const& T>
bool TemplateEngine<T>::spawnBotControl(Control*& c) {
    auto&& point =
        getSafeSpawnFromInflu(T.BOT_SPAWN_SIZE, T.PLAYER_SAFE_SPAWN_RADIUS);
    if (point.safe) {
        auto& cell = newCell();
        ...
        influences.push_back(
            {c->viewport.x, c->viewport.y, T.BOT_SPAWN_SIZE * 1.25f, c});
    }
    return point.safe;
}
...
bool TemplateEngine<T>::spawnPlayerControl(Control*& c) {
    ...
    if (c->handle->dual && c->handle->dual->control &&
        c->handle->dual->control->alive)
        target = c->handle->dual->control;

    cell_cord_prec spawnSize = c->overwrites.spawn * T.PLAYER_SPAWN_SIZE;

    if constexpr (T.PLAYER_INIT_SPAWN_SIZE > 0) {
        if (!c->handle->isAlive())
            spawnSize = c->overwrites.spawn * T.PLAYER_INIT_SPAWN_SIZE;
    }

    auto&& point = getPlayerSpawnPoint(
        target, spawnSize,
        target ? target->lastSplit - __now < 1000 * 1000 * 1000 : false);
    if (point.safe) {
        auto& cell = newCell();
        ...
        c->viewport.hh = T.PLAYER_VIEW_MIN * T.PLAYER_VIEW_SCALE;
        c->aabb = cell.shared.aabb;
        c->score = spawnSize * spawnSize * 0.01f;

        influences.push_back(
            {c->viewport.x, c->viewport.y, spawnSize * 1.25f, c});
    }

    return point.safe;
}

bool Engine::freeHandle(Handle* handle) {
    for (auto h : handles) {
...
template <OPT const& T>
void TemplateEngine<T>::handleInput(float dt) {
    unordered_map<uint16_t, Control*> copy = controls;

    cell_cord_prec playerMass = 0.;
    cell_cord_prec botMass = 0.;
    const cell_cord_prec restartMass =
        map.hw * map.hh / 100. * T.WORLD_RESTART_MULT;
    ...
    for (auto [id, c] : copy) {
        ...
        if (!c->cells.size() && !c->handle) {
            ...
            spawnSet.erase(c);
            killArray.erase(
                std::remove_if(killArray.begin(), killArray.end(),
                               [c](auto pair) { return pair.first == c; }),
                killArray.end());
            controls.erase(id);
        ...
        influences.push_back(
            {c->viewport.x, c->viewport.y, sqrtf(c->score * 100) * 1.25f, c});

        if (c->handle->isBot())
            botMass += c->score;
        else {
            playerMass += c->score;
            aliveControls.push_back(c);
        }
    ...
    for (uint32_t _ = 0; _ < server->threadPool.size(); _++) {
        server->threadPool.enqueue([&] {
            ...
            local_ejected.reserve(T.PLAYER_MAX_CELLS * queue_size /
            ...
            while (true) {
                ...
                if constexpr (T.NORMALIZE_THRESH_MASS > 0.f) {
                    const cell_cord_prec multi =
                        std::max(sqrt(c->score / T.NORMALIZE_THRESH_MASS),
                                 cell_cord_prec(1));
                ...
                if (c->score > T.PLAYER_SPLIT_CAP_T1)
                    expireTick = 9;
                else if (c->score > T.PLAYER_SPLIT_CAP_T2)
                    expireTick = 7;
                ...
                for (auto& s : c->splitAttempts) {
                    ...
                    for (auto cell : copy) {
                        ...
                        if constexpr (T.NORMALIZE_THRESH_MASS > 0.) {
                            const cell_cord_prec multi2 = std::max(
                                cell->r / splitRadiusThresh, cell_cord_prec(1));
                            auto b =
                                std::min(multi2 * boost, T.PLAYER_MAX_BOOST);
                            ...
                            if constexpr (T.EX_FAST_BOOST_R > 0.) {
                                auto scaledBoost =
                                    cell->r > T.EX_FAST_BOOST_R
                                        ? (sqrtf(cell->r / T.EX_FAST_BOOST_R) *
                                           boost)
                                        : boost;
                                auto n =
                                    splitFromCell(cell, cell->r * M_SQRT1_2,
                                                  {dx, dy, scaledBoost});
                                c->cells.push_back(n);
                            } else {
                                auto n = splitFromCell(
                                    cell, cell->r * M_SQRT1_2, {dx, dy, boost});
                                c->cells.push_back(n);
                            }
                        }

                        if (s.attempt <= 0 && T.PLAYER_SPLIT_ADD_AGE) {
                            cell->age += T.PLAYER_SPLIT_ADD_AGE;
                        }
                    }
                ...
                c->splitAttempts.erase(
                    std::remove_if(
                        c->splitAttempts.begin(), c->splitAttempts.end(),
                        [&](SplitAttempt& s) {
                            return s.attempt <= 0 || s.tick >= expireTick;
                        }),
                    c->splitAttempts.end());
                ...
                constexpr uint64_t noEjectPopDelayNano =
                    T.PLAYER_NO_EJECT_POP_DEALY * MS_TO_NANO;

                auto factor =
                    1.f / powf(2.718281828459045f, c->cells.size() / 1000.f);

                cell_cord_prec overwriteEjectMulti = c->overwrites.eject;
                cell_cord_prec ejectLossSqr =
                    T.EJECT_LOSS * T.EJECT_LOSS * (factor * factor);
                cell_cord_prec ejectSizeMin = T.PLAYER_MIN_EJECT_SIZE * factor;

                const cell_cord_prec ejectSize =
                    overwriteEjectMulti * T.EJECT_SIZE * factor;
                const cell_cord_prec ejectBoost =
                    c->overwrites.boost * T.EJECT_BOOST;
                bool ejectPerk = false;
                ...
                if (T.PLAYER_NO_EJECT_POP_DEALY > 0 &&
                    ...
                    while (c->lastEject <= __now + dt * MS_TO_NANO &&
                        ...
                        for (auto cell : c->cells) {
                            ...
                            if (r < ejectSizeMin ||
                                cell->age < T.PLAYER_NO_EJECT_DELAY)
                                continue;
                            ...
                            if constexpr (T.EJECT_DISPERSION > 0.f) {
                                std::uniform_real_distribution<cell_cord_prec>
                                    randomEject(-T.EJECT_DISPERSION,
                                                T.EJECT_DISPERSION);
                                const cell_cord_prec angle =
                                    atan2f(dx, dy) + randomEject(generator);
                                dx = sinf(angle);
                                dy = cosf(angle);
                            }
                            ...
                            auto cid = cell_id(n);
                            boosts[cid] = {dx, dy, ejectBoost};

                            if (ejectPerk) {
                                if (rngBool(generator)) {
                                    n.type = CYT_TYPE;
                                    n.data = 1;
                                    cyts.push_back(&n);
                                } else {
                        ...
                        uint64_t newT =
                            __now + T.EJECT_DELAY * MS_TO_NANO * ejectedCount;

                        // WTF: floating point error makes solotrick actually OP
                        // but breaks after 7 days uint64_t oldT = __now +
                        // T.EJECT_DELAY * MS_TO_NANO_F * ejectedCount;

                        // if (newT != oldT) logger::debug("Diff: %i ms\n",
                        // ((long) newT - (long) oldT)); c->lastEject = newT +
                        // jitter(generator);
                        c->lastEject = newT;
                        // c->lastEject = oldT;
                    }
                ...
                // Spawn
                if (c->canSpawn()) {
                    auto dt = ((__now - std::max(c->lastSpawned, c->lastDead)) /
                               1000 / 1000) /
                              1000.f;
            ...
            // Append vector
            std::scoped_lock lock(m);
            ejected.insert(ejected.end(), local_ejected.begin(),
                           local_ejected.end());
        });
    }
...
template <OPT const& T>
void TemplateEngine<T>::handleOutput(float dt) {
    mutex qm;

    auto hcopy = handles;

    hcopy.sort([](auto a, auto b) { return a->viewArea < b->viewArea; });
...
void TemplateEngine<T>::updateCells(float dt) {
    ...
    for (auto cell : exps) {
        ...
        if (boostCell(*cell, dt)) {
            ...
            auto cid = cell_id(cell);
            boosts[cid].d = T.EJECT_BOOST;

            cell->updateAABB();
            tree->update(cell);

            if (cell->age > T.PERK_DYNAMIC_MAX_AGE) cell->flag |= REMOVE_BIT;
        }
    }
    ...
    const cell_cord_prec globalMulti =
        1.f + playerMass / mapMass * T.GLOBAL_DECAY;
        ...
        server->threadPool.enqueue([&] {
            while (true) {
                ...
                if constexpr (T.ANTI_CAMP_TIME > 0) {
                    constexpr cell_cord_prec camp =
                        T.ANTI_CAMP_TIME * 1000 * MS_TO_NANO;
                    const cell_cord_prec nosplit = __now - c->lastSplit;
                    if (nosplit > camp && c->score > T.ANTI_CAMP_MASS) {
                        cell_cord_prec campMulti =
                            nosplit / MS_TO_NANO_F / 1000;
                        if (c->handle) {
                            c->dynamicViewportFactor =
                                std::max(0., 1. - T.ANTI_CAMP_MULT * campMulti);
                        }
                    } else {
                ...
                // Remove extra cells
                if (c->overwrites.cells > 0 &&
                    c->cells.size() > c->overwrites.cells) {
                    for (int i = c->overwrites.cells; i < c->cells.size();
                         i++) {
                        auto cell = c->cells[i];
                        tree->remove(cell);
                        memset(cell, 0, sizeof(Cell));
                ...
                auto campDecay = c->score > T.ANTI_CAMP_MASS;
                ...
                for (auto cell : cellsCopy) {
                    ...
                    // Decay
                    if (cell->r > T.DECAY_MIN) {
                        cell_cord_prec extraFactor = 1.;
                        if constexpr (T.ANTI_CAMP_TIME) {
                            constexpr cell_cord_prec camp =
                                T.ANTI_CAMP_TIME * 1000;
                            if (campDecay && cell->age > camp) {
                                extraFactor = std::min(
                                    (1. + T.ANTI_CAMP_MULT * 0.001 * cell->age),
                                    4.);
                            }
                        }

                        cell->r -= extraFactor * decay * decayMulti * cell->r *
                                   staticDecay * dt * 0.0001f;
                    }

                    cell->r = cell->r > maxC   ? maxC
                              : cell->r < minC ? minC
                                               : cell->r;

                    // Calculate collision bit
                    if (c->overwrites.canColli &&
                        cell->age > T.PLAYER_NO_COLLI_DELAY)
                        cell->flag |= COLL_BIT;

                    // Calculate autosplit
                    if constexpr (T.PLAYER_AUTOSPLIT_SIZE > 0) {
                        if (c->overwrites.canAuto &&
                            cell->r > T.PLAYER_AUTOSPLIT_SIZE) {
                            cell_cord_prec angle = rngAngle();
                            auto n = splitFromCell(cell, cell->r * M_SQRT1_2,
                                                   {sinf(angle), cosf(angle),
                                                    T.PLAYER_SPLIT_BOOST});
                            c->cells.push_back(n);
                        }
                    }
                    ...
                    if constexpr (T.PLAYER_MERGE_TIME > 0) {
                        constexpr cell_cord_prec mergeIncrease =
                            T.PLAYER_MERGE_INCREASE;
                        initial = 10000. * T.PLAYER_MERGE_TIME;
                        const cell_cord_prec increase =
                            100. * cell->r * mergeIncrease;

                        if constexpr (T.PLAYER_MERGE_NEW_VER) {
                            mergeTime = instant ? (T.PLAYER_NO_COLLI_DELAY +
                                                   T.ULTRA_MERGE_DELAY)
                                                : std::max(increase, initial);
                        } else {
                    ...
                    if (c->overwrites.canMerge &&
                        (cell->age > mergeTime ||
                         (T.EX_FAST_MERGE_MASS &&
                          (cell->r < T.PLAYER_MIN_EJECT_SIZE &&
                           c->score > T.EX_FAST_MERGE_MASS &&
                           cell->age > initial)))) {
                        cell->flag |= MERGE_BIT;
                    }
...
void TemplateEngine<T>::resolve(float dt) {
    ...
    for (auto [_, c] : controls)
        if (c->cells.size()) temp.push_back(c);
    std::sort(temp.begin(), temp.end(),
              [](auto c1, auto c2) { return c1->score > c2->score; });
            ...
            while (true) {
                ...
                std::sort(c->sorted.begin(), c->sorted.end(),
                          [](auto a, auto b) {
                              ...
                              // else
                              return a->r - a->age * 0.1 < b->r - b->age * 0.1;
                          });

                if (!c->overwrites.canMerge && !c->overwrites.canColli)
                    continue;
                ...
                for (auto cell : c->sorted) {
                    ...
                    tree->query(*cell, false, [&](Cell* other, uint32_t level) {
                        ...
                        // This flag is only written to from the same thread, no
                        // need for atomic rw here
                        ...
                        if (type == other->type) {
                            ...
                            if (flagsAND & MERGE_BIT)
                                action = Action::MERGE;
                            else {
                                if constexpr (T.ULTRA_MERGE) {
                                    if ((flags | otherFlags) & COLL_BIT)
                                        action = Action::COL;
                                } else {
                                    if (flagsAND & COLL_BIT)
                                        action = Action::COL;
                                }
                            }
                        }
                        ...
                        if (action == Action::EAT && cell->r < r2 * T.EAT_MULT)
                            return;
                        ...
                        if (action == Action::COL) {
                            ...
                            // if (d + cell->r < other->r) cell->flag |=
                            // INSIDE_BIT; if (d + r2 < cell->r) other->flag |=
                            // INSIDE_BIT;
                            ...
                            cell_cord_prec m1 =
                                (m < cell->r ? m : cell->r) * aM;
                            cell->x -= dx * m1;
                            cell->y -= dy * m1;

                            if constexpr (T.COLLI_RATIO > 1) {
                                if (cell->r / other->r > T.COLLI_RATIO ||
                                    other->r / cell->r > T.COLLI_RATIO)
                                    return;
                            }
                            ...
                            // constexpr cell_cord_prec MIN_RELAX =
                            // T.PLAYER_MIN_EJECT_SIZE * 3.0f; if (instant ||
                            // cell->r > T.RELAXATION_RATIO_THRESH * r2 || r2 <
                            // MIN_RELAX) {
                            //     cell_cord_prec m1 = (m < cell->r ? m :
                            //     cell->r) * aM; cell->x -= dx * m1; cell->y -=
                            //     dy * m1;

                            //     constexpr bool boostCutoff =
                            //     (T.NEW_BOOST_ALGO ? 0.f : 1.f); const
                            //     cell_cord_prec modifier = cell->boost.d >
                            //     boostCutoff ? 1.5f : 1.f;

                            //     cell_cord_prec m2 = (m < r2 ? m : r2) * bM *
                            //     modifier; other->x += dx * m2; other->y += dy
                            //     * m2;
                            // } else {
                            //     cell_cord_prec m1 = (m < cell->r ? m :
                            //     cell->r) * aM * T.M1_RELAXATION; cell->x -=
                            //     dx * m1; cell->y -= dy * m1;

                            //     cell_cord_prec m2 = (m < r2 ? m : r2) * bM *
                            //     T.M2_RELAXATION; other->x += dx * m2;
                            //     other->y += dy * m2;
                            // }
    ...
    server->threadPool.enqueue([&] {
        for (auto cell : deadCells) {
            ...
            tree->query(*cell, false, [&](Cell* other, uint32_t) {
                uint16_t otherFlags = other->flag;
    ...
    for (uint32_t i = 0; i < server->threadPool.size(); i++) {
        server->threadPool.enqueue([&] {
            ...
            while (true) {
                ...
                // uint16_t dualType = 0;
                // if (c->handle && c->handle->dual && c->handle->dual->control)
                // dualType = c->handle->dual->control->id;
                if (!c->handle) continue;
                ...
                if (c->handle->canEatPerk()) {
                    ...
                    for (auto cell : c->sorted) {
                        ...
                        tree->query(*cell, false, [&](Cell* other, uint32_t) {
                            ...
                            uint16_t otherFlags =
                                other->flag.load(std::memory_order_relaxed);

                            if (otherFlags & SKIP_RESOLVE_BITS) return;
                            if (cell->r < other->r) return;

                            if (type == other->type) return;
                            if (!(cell->flag & NOEAT_BIT) &&
                                (otherFlags & NOEAT_BIT))
                                return;

                            if (other->age <= 60) return;

                            cell_cord_prec r2 = other->r;
                            // Basic condition to eat
                            if (other->type != WH_TYPE &&
                                cell->r < r2 * T.EAT_MULT)
                                return;
                            ...
                            if (!other->flag.compare_exchange_weak(
                                    otherFlags,
                            ...
                            if (other->type == EXP_TYPE) {
                                c->handle->perks.exps += uint16_t(other->data);
                            } else if (other->type == CYT_TYPE) {
                                c->handle->perks.cyts += uint16_t(other->data);
                            } else {
                                cell->r = sqrt(cell->r * cell->r + r2 * r2);
                                cell->flag |=
                                    UPDATE_BIT |
                                    (other->type == WH_TYPE ? POP_BIT : 0);
                            }
                        });
                    }
                } else {
        ...
        server->threadPool.enqueue([&] {
            ...
            for (auto cell : wormholes) {
                ...
                tree->query(*cell, false, [&](Cell* other, uint32_t) {
                    ...
                    // if (other->type == WH_TYPE || other->type == EXP_TYPE ||
                    // other->type == CYT_TYPE) return;
                    if (!(cell->flag & NOEAT_BIT) && (otherFlags & NOEAT_BIT))
                        return;

                    cell_cord_prec r1 = cell->r;
                    cell_cord_prec r2 = other->r;

                    cell_cord_prec dx = cell->x - other->x;
                    cell_cord_prec dy = cell->y - other->y;

                    cell_cord_prec rSum = r1 + r2;
                    cell_cord_prec dSqr = dx * dx + dy * dy;

                    auto force = T.WH_GRAVITY * r1 * r1 / dSqr;

                    cell_cord_prec d = sqrt(dSqr);

                    if (d > r1 * T.WH_EXPANSION) return;

                    auto oid = cell_id(other);
                    auto& boost = boosts[oid];

                    Boost gravity = {.x = dx / d, .y = dy / d, .d = force};
                    Boost angular = {
                        .x = -dy / d,
                        .y = dx / d,
                        .d = force * 4 * (1 - d / r1 * T.WH_EXPANSION)};

                    boost += gravity;
                    boost += angular;

                    boost.d = std::min(T.PLAYER_MAX_BOOST, boost.d);

                    // printf("boost: %.3f, %.3f, %.3f\n", boost.x, boost.y,
                    // boost.d);
                    ...
                    if (cell->flag & SKIP_RESOLVE_BITS) return;
                    if (!other->flag.compare_exchange_weak(
                            otherFlags, uint16_t(otherFlags | REMOVE_BIT),
                            std::memory_order_release,
                            std::memory_order_relaxed))
                        return;
                    ...
                    cell->r = sqrt(cell->r * cell->r + r2 * r2 * 0.01);
                    cell->flag |= UPDATE_BIT;
                });
                // Size cap
                cell->r = std::min(cell->r, cell_cord_prec(T.WH_CELL_MAX_SIZE));
            }
        });
        ...
        server->threadPool.enqueue([&] {
            while (true) {
                ...
                for (auto cell : c->sorted) {
                    ...
                    auto cid = cell_id(cell);
                    auto& cell_boost = boosts[cid];

                    Grid_EV.query(
                        cell->shared.aabb,
                        [&](Cell* other) {
                            if (skipOthers &&
                                type != (other->type & PELLET_TYPE))
                                return;
                            auto otherFlags =
                                other->flag.load(std::memory_order_relaxed);
                            ...
                            cell_cord_prec dy = other->y - cell->y;
                            cell_cord_prec d = sqrt(dx * dx + dy * dy);
                            if ((cell->r > r2 * T.EAT_MULT) &&
                                (d < cell->r - r2 / T.EAT_OVERLAP)) {
                                cell->r = sqrt(cell->r * cell->r + r2 * r2);
                                ...
                                if (other->type == VIRUS_TYPE) {
                                    ...
                                    auto oid = cell_id(other);
                                    auto other_boost = boosts[oid];

                                    // Boost player cell
                                    if constexpr (T.NEW_BOOST_ALGO) {
                                        if (cell_boost.d <=
                                            T.PLAYER_MAX_BOOST) {
                                            cell_boost.x *= cell_boost.d;
                                            cell_boost.y *= cell_boost.d;

                                            auto& x0 = cell_boost.x;
                                            auto& y0 = cell_boost.y;

                                            auto& x1 = other_boost.x;
                                            auto& y1 = other_boost.y;

                                            auto dot = x1 * x0 + y1 * y0;
                                            auto mag_sq = x0 * x0 + y0 * y0;
                                            auto proj = dot / mag_sq;

                                            x0 *= (1 + proj * T.BOOST_AMOUNT);
                                            y0 *= (1 + proj * T.BOOST_AMOUNT);
                                            ...
                                            cell_boost.normalize();
                                        }
                                    } else {
                                        cell_cord_prec ratio =
                                            other->r / (cell->r + 100.f);
                                        cell_boost.d +=
                                            ratio * 0.025f * other_boost.d;
                                        if (cell_boost.d >= T.PLAYER_MAX_BOOST)
                                            cell_boost.d = T.PLAYER_MAX_BOOST;

                                        cell_cord_prec bx =
                                            cell_boost.x +
                                            ratio * 0.02f * other_boost.x;
                                        cell_cord_prec by =
                                            cell_boost.y +
                                            ratio * 0.02f * other_boost.y;
                                        cell_cord_prec norm =
                                            1.f / sqrtf(bx * bx + by * by);
                                        cell_boost.x = bx * norm;
                                        cell_boost.y = by * norm;
                                    }
        ...
        for (auto cell : wormholes) {
            ...
            Grid_EV.query(
                ...
                [&](Cell* other) {
                    ...
                    auto force = T.WH_GRAVITY * r1 * r1 * 10 / dSqr;
            ...
            while (true) {
                ...
                for (auto cell : c->sorted) {
                    ...
                    if (f & REMOVE_BIT) {
                        ...
                        const bool noPop = c->overwrites.cells == 1;
                        auto maxCells = c->overwrites.cells;
                        if (maxCells <= 0) maxCells = T.PLAYER_MAX_CELLS;

                        if (!noPop) {
                            distributeMass<T>(maxCells - c->cells.size(),
                                              cell->r * cell->r * 0.01f,
                                              [&](cell_cord_prec mass) {
                                                  cell_cord_prec angle =
                                                      rngAngle();
                                                  auto n = splitFromCell(
                                                      cell, sqrtf(mass * 100),
                                                      {sinf(angle), cosf(angle),
                    ...
                    if (cell->flag & LOCK_BIT) {
                        ...
                        cell->x = (bL * (bL * x0 - aL * y0) - aL * cL) *
                                  c->abSqrSumInvL;
                        cell->y = (aL * (-bL * x0 + aL * y0) - bL * cL) *
                                  c->abSqrSumInvL;
                    }
                    tree->update(cell);
                }
            ...
            if (other->type == VIRUS_TYPE) {
                if (r * T.EAT_MULT < r2 && d < r2 - r / T.EAT_OVERLAP) {
                    ...
                    if constexpr (T.VIRUS_MAX_SIZE > 0.f) {
                        if (other->r > T.VIRUS_MAX_SIZE) {
                            if constexpr (T.VIRUS_EXPLODE) {
                                ...
                                other_boost.x = e_boost.x;
                                other_boost.y = e_boost.y;
                            } else {
                    ...
                    if constexpr (T.VIRUS_PUSH_BOOST > 0.f) {
                        const cell_cord_prec newBoost = std::min(
                            T.VIRUS_MAX_BOOST, e_boost.d + other_boost.d);
                        const cell_cord_prec bx =
                            other_boost.x * other_boost.d +
                            e_boost.x * e_boost.d;
                        const cell_cord_prec by =
                            other_boost.y * other_boost.d +
                            e_boost.y * e_boost.d;
                        const cell_cord_prec norm = sqrt(bx * bx + by * by);
                        if (norm > 0.f) {
                            other_boost.x = bx / norm;
                            other_boost.y = by / norm;
                            other_boost.d = newBoost;
                        }
                    }
...
void TemplateEngine<T>::postResolve() {
    for (auto v : virusToSplit) {
        if constexpr (T.VIRUS_SPLIT_BOOST) {
            auto vid = cell_id(v);
            auto v_boost = boosts[vid];
            auto angle = atan2(v_boost.x, v_boost.y);
            auto& n = newCell();
            ...
            auto nid = cell_id(n);
            boosts[nid] = {sinf(angle), cosf(angle), T.VIRUS_SPLIT_BOOST};
            ...
            for (int i = 0; i < splitTimes; i++) {
                ...
                n.r = T.VIRUS_SIZE;
                n.type = VIRUS_TYPE;
                std::uniform_real_distribution<cell_cord_prec> boost_dist(5.,
                                                                          20.);

                auto nid = cell_id(n);
                boosts[nid] = {sinf(angle), cosf(angle),
                               boost_dist(generator) * T.VIRUS_SPLIT_BOOST};
    ...
    filterCells(wormholes, removedCells);
}
...
bool TemplateEngine<T>::boostCell(Cell& cell, float& dt) {
    ...
    if constexpr (T.NEW_BOOST_ALGO) {
        if (cell_boost.d > 0.) {
            const cell_cord_prec od = cell_boost.d;
            const cell_cord_prec nd =
                (cell_boost.d = std::max(cell_boost.d - T.BOOST_DELAY * dt,
                                         cell_cord_prec(0)));
            const cell_cord_prec dist =
                (100. * T.BOOST_MULTI) * (exp((0.0025 * T.BOOST_FACTOR) * od) -
                                          exp((0.0025 * T.BOOST_FACTOR) * nd));

            cell.x += cell_boost.x * dist;
            cell.y += cell_boost.y * dist;
            cell.flag |= UPDATE_BIT;
        ...
        if (cell_boost.d > 1.) {
            const cell_cord_prec db =
                cell_boost.d * (T.BOOST_FACTOR * 0.0025) * dt;
            cell.x += cell_boost.x * db;
            cell.y += cell_boost.y * db;
            cell_boost.d -= db;
            cell.flag |= UPDATE_BIT;
...
void TemplateEngine<T>::bounceCell(Cell& cell) {
    ...
    if constexpr (T.STRICT_BORDER) {
        box = AABB{cell.r - map.hw, map.hw - cell.r, cell.r - map.hh,
                   map.hh - cell.r};
    } else {
        box = AABB{cell.r / 2.f - map.hw, map.hw - cell.r / 2.f,
                   cell.r / 2.f - map.hh, map.hh - cell.r / 2.f};
    }
...
template <OPT const& T>
void TemplateEngine<T>::movePlayerCell(Cell& cell, float& dt,
                                       cell_cord_prec& mouseX,
                                       cell_cord_prec& mouseY,
                                       uint8_t& lineLocked, uint16_t& flags,
                                       cell_cord_prec& multi) {
    if (lineLocked) {
        flags |= cell.flag;
        cell.flag |= LOCK_BIT;
    }
...
template <OPT const& T>
Point TemplateEngine<T>::randomPoint(cell_cord_prec size, cell_cord_prec xmin,
                                     cell_cord_prec xmax, cell_cord_prec ymin,
                                     cell_cord_prec ymax) {
...
template <OPT const& T>
BoolPoint TemplateEngine<T>::getPlayerSpawnPoint(Control* target,
                                                 cell_cord_prec spawnSize,
                                                 bool avoidCenter) {
    ...
    if (!target) {
        ...
        if (rand() % 3) {
            std::uniform_int_distribution<int> picker(0,
                                                      aliveControls.size() - 1);
    ...
    const cell_cord_prec f1 =
        std::max(T.PLAYER_VIEW_MIN, cell_cord_prec(1.2) * (vx - box.l));
    const cell_cord_prec f2 =
        std::max(T.PLAYER_VIEW_MIN, cell_cord_prec(1.2) * (box.r - vx));
    const cell_cord_prec f3 =
        std::max(T.PLAYER_VIEW_MIN, cell_cord_prec(1.2) * (vy - box.b));
    const cell_cord_prec f4 =
        std::max(T.PLAYER_VIEW_MIN, cell_cord_prec(1.2) * (box.t - vy));
...
template <OPT const& T>
BoolPoint TemplateEngine<T>::getSafeSpawnPoint(cell_cord_prec size,
...
template <OPT const& T>
BoolPoint TemplateEngine<T>::getSafeSpawnFromInflu(cell_cord_prec size,
                                                   cell_cord_prec safeSize) {
    uint32_t tries = T.MAX_SPAWN_TRIES;
    cell_cord_prec safe = size * safeSize;
    while (--tries) {
...
template <OPT const& T>
Cell* TemplateEngine<T>::splitFromCell(Cell* cell, cell_cord_prec size,
                                       Boost boost) {
...
void TemplateEngine<T>::syncState() {
    ...
    for (int i = 0; i < T.CELL_LIMIT; i++) {
        ...
        if (cell.type == CYT_TYPE) {
            ...
            exps.push_back(&cell);
            tree->insert(&cell);
        } else if (cell.type == WH_TYPE) {
            wormholes.push_back(&cell);
            tree->insert(&cell);
